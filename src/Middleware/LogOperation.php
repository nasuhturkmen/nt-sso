...
use NasuhTurkmen\Admin\Auth\Database\OperationLog as OperationLogModel;
...
class LogOperation
{
    ...
    {
        $filter = config('sso.operation_log.filter_input', []);
    ...
    {
        return config('sso.operation_log.enable')
    ...
    {
        $allowedMethods = collect(config('sso.operation_log.allowed_methods'))->filter();
    ...
    {
        foreach (config('sso.operation_log.except') as $except) {
            if ($except !== '/') {
                $except = trim($except, '/');
            }
